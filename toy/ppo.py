        ...
        def _update_step(runner_state, unused):
            ...
            if config.get("USE_WANDB") and config.get("DEBUG"):
                def wandb_callback(info, update_step_val):
                    ...
                    if returned_episode.sum() > 0:
                        returns = info["returned_episode_returns"][returned_episode]
                        lengths = info["returned_episode_lengths"][returned_episode]
                        ...
                        # Log to wandb
                        wandb.log({
                            "charts/episodic_return": returns.mean(),
                            "charts/episodic_length": lengths.mean(),
...
if __name__ == "__main__":
    config = {
        ...
        "DEBUG": True,
        "USE_WANDB": True,
        "WANDB_PROJECT": "toy-ppo",
        "WANDB_ENTITY": None,  # Set to your wandb username/team
    }

    # Initialize wandb
    if config["USE_WANDB"]:
        run_name = f"{config['ENV_NAME']}-{int(config['TOTAL_TIMESTEPS'] // 1e3)}K"
        wandb.init(
