...
#         raise ImportError(
#             "Pallas is not available. Install it with pip install jax[experimental]"
#         )
...
#         inputs, ((0, 0), (0, padded_input_features - input_features))
#     )
#     padded_kernels = jnp.pad(
#         kernels,
#         (
#             (0, 0),
...
#     if use_bias:
#         padded_biases = jnp.pad(
#             biases, ((0, 0), (0, padded_output_features - output_features))
#         )
#     else:
#         padded_biases = None
...
#     padded_output_shape = jax.ShapeDtypeStruct(
#         (batch_size, padded_output_features), inputs.dtype
#     )
...
class MoEDense(nn.Module):
    ...
    def __call__(self, inputs: Array, task_idx: int) -> Array:
        ...
        kernels = self.param("kernel", self.kernel_init, kernel_shape, self.param_dtype)

        if self.use_bias:
            bias_shape = (self.n_tasks, self.features)
            biases = self.param("bias", self.bias_init, bias_shape, self.param_dtype)
        else:
            biases = None
...
trace_dir = "/home/renos/tensorboard"
with profiler.trace(trace_dir, create_perfetto_link=False, create_perfetto_trace=True):
    ...
    init_task_ids = jnp.zeros(
        (1,), dtype=jnp.int32
    )  # Shape is (1,) for a single example
    ...
    print(
        f"Average inference time over {runs} runs: {avg_inference_time*1000:.4f} ms per batch"
    )
