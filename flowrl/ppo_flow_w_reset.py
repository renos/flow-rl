...
def make_train(config, prev_model_state=None, return_test_network=False):
    ...
    def train(rng):
        ...
        # Initialize per-env latest snapshot tracking
        ex_state = {
            **ex_state,
        ...
        # Helper to mask-broadcast over leading env dimension
        ...
        # Update 'latest snapshot' when player_state increases
        def update_latest_snapshot(ex_state, env_state, obs):
            new_player_state = env_state.env_state.player_state
            ...
            if config.get("PROGRESSIVE_RESET_CURRICULUM", False):
                ...
                is_relevant = new_player_state <= current_reset_state
                improved = jnp.logical_and(new_player_state > prev_best, is_relevant)
            else:
            ...
            best = jnp.where(improved, new_player_state, prev_best)
            return {
                **ex_state,
                "best_player_state": best,
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                # Track latest snapshot whenever the environment "state" increases
                ex_state = update_latest_snapshot(ex_state, env_state, obsv)

                # If this step terminated and reset to 0, optionally restore to latest snapshot.
                # Otherwise, keep the reset and also reset the snapshot to this new 0 state.
                player_state_after = env_state.env_state.player_state
                was_zero_reset = jnp.logical_and(done, player_state_after == 0)
                ...
                actual_success_transition = jnp.logical_and(
                    done, player_state_after == (player_state + 1)
                )
                actual_failure_transition = jnp.logical_and(
                    done, player_state_after == 0
                )
                ...
                if config.get("PROGRESSIVE_RESET_CURRICULUM", False):
                    ...
                    current_reset_state = ex_state.get("progressive_reset_state", 0)
                    was_goal_state = jnp.logical_and(done, player_state_after == current_reset_state + 1)
                    ...
                    was_goal_state = jnp.logical_and(done, player_state_after == config["SUCCESS_STATE_INDEX"])
                    zero_or_goal = jnp.logical_or(was_zero_reset, was_goal_state)
                ...
                if config.get("LATEST_RESET_PROB", 0.0) > 0:
                    ...
                    choose_latest = jax.random.bernoulli(
                        rng_choice,
                        p=config["LATEST_RESET_PROB"],
                        shape=reset_gate.shape,
                    )
                ...
                if config.get("LATEST_RESET_REBASE_ON_FRESH", True):
                    ex_state = {
                        **ex_state,
                        "best_player_state": apply_mask(
                            keep_reset, player_state_after, ex_state["best_player_state"]
                        ),
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(carry, batch_info):
                    ...
                    def _loss_fn(params, traj_batch, gae, targets):
                        ...
                        gae_raw = gae
                        # Skill ids for optional per-skill ops and frontier weighting
                        ...
                        # Per-skill dynamic balancing: upweight skills inversely to their minibatch share
                        if config.get("PER_SKILL_BALANCE", True):
                            num_samples = skill_ids.shape[0]
                            per_skill_counts = jnp.bincount(skill_ids, length=num_heads).astype(jnp.int32)
                            threshold = jnp.int32(config.get("PER_SKILL_BALANCE_THRESHOLD", 64))
                            cap = jnp.float32(config.get("PER_SKILL_BALANCE_CAP", 4.0))
                            # scale_k = clip(num_samples / count_k, 1, cap) if count_k >= threshold else 1
                            counts_f = jnp.maximum(per_skill_counts.astype(jnp.float32), 1.0)
                            raw_scale = jnp.clip(num_samples / counts_f, 1.0, cap)
                            enable = (per_skill_counts >= threshold).astype(jnp.float32)
                            per_skill_scale = 1.0 + (raw_scale - 1.0) * enable
                            weights = per_skill_scale[skill_ids]
                        else:
                        ...
                        vloss_per_sample = value_loss_per_sample
...
if __name__ == "__main__":
    ...
    parser.add_argument("--success_rate_ema_alpha", type=float, default=0.15,
                        help="EMA smoothing factor for per-skill success rate (0.1=slow, 0.3=fast adaptation)")
    ...
    # Latest-state reset probability (argument only; behavior to be added next)
    ...
    # Progressive curriculum: unlock states sequentially as agent masters them
    parser.add_argument("--progressive_reset_curriculum", action=argparse.BooleanOptionalAction, default=False,
                        help="If true, only reset at current unlocked state; advance when success rate > threshold.")
    parser.add_argument("--progressive_reset_threshold", type=float, default=0.2,
                        help="Success rate threshold (0-1) to advance to next state in progressive curriculum (default: 0.2)")
