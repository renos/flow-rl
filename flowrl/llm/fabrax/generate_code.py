...
from flowrl.llm.fabrax.verify_code import verify_function
import os
...
def generate_py_file(data, file_path="generated_file.py"):
    ...
    with open(file_path, "w") as f:
        f.write("from craftax.fabrax.constants import *\n")
        f.write("from craftax.fabrax.envs.craftax_state import Inventory\n")
        f.write("import jax\n")
...
def create_inventory_from_array(inventory_array):
    return Inventory(
        ...
        copper=round(inventory_array[12]),
        tin=round(inventory_array[13]),
        sand=round(inventory_array[14]),
        clay=round(inventory_array[15]),
        limestone=round(inventory_array[16]),
        leather=round(inventory_array[17]),
        iron_bar=round(inventory_array[18]),
        steel_bar=round(inventory_array[19]),
        bronze_bar=round(inventory_array[20]),
        glass=round(inventory_array[21]),
        brick=round(inventory_array[22]),
        lime=round(inventory_array[23]),
        tar=round(inventory_array[24]),
        bottle=round(inventory_array[25]),
        lens=round(inventory_array[26]),
        telescope=round(inventory_array[27]),
        mortar=round(inventory_array[28]),
        fertilizer=round(inventory_array[29]),
        flux=round(inventory_array[30]),
        steel_pickaxe=round(inventory_array[31]),
        bronze_pickaxe=round(inventory_array[32]),
        steel_sword=round(inventory_array[33]),
        bronze_sword=round(inventory_array[34]),
        tonic_basic=round(inventory_array[35]),
        tonic_stoneskin=round(inventory_array[36]),
    )