...
def get_ctxt():
    ...
    pickle_path = os.path.join(current_directory, "crafter_initial_QA.pkl")
    ...
    for k, v in QA_data.items():
        ...
        print(
            "\n".join(
                [
                    "{}{}. {}".format("-> " if i in choosen_idx[k] else "   ", i, x)
                    for i, x in enumerate(v["questions"])
                ]
            )
        )
    ctxt_path = os.path.join(current_directory, "cache/ctxt.pkl")
    if os.path.exists(ctxt_path):
        with open(ctxt_path, "rb") as f:
            CTXT = pickle.load(f)
    else:
        import itertools
        ...
        def get_list(L, idx):
            ...
            if type(L[0]) == str:
                ...
                return list(
                    itertools.chain.from_iterable([get_list(ll, idx) for ll in L])
                )
    ...
    CTXT = CTXT.replace(
        ...
        "Move North: Flat ground to the north of the agent.",
    )
    CTXT = CTXT.replace(
        "Move Down: Flat ground below the agent.",
        "Move South: Flat ground to the south of the agent.",
    )
    CTXT = CTXT.replace(
        "Move Left: Flat ground left to the agent.",
        "Move West: Flat ground to the west of the agent.",
    )
    CTXT = CTXT.replace(
        "Move Right: Flat ground right to the agent.",
        "Move East: Flat ground to the east of the agent.",
    )
    ...
    CTXT = CTXT.replace(
        ...
        "List of game achievements and their requirements:",
    )
    CTXT = CTXT.replace(
        "Write all actions as a numbered list. For each action, list its requirements.",
        "List of all actions and their requirements:",
    )
...
def describe_achievements(info, CTXT):
    ...
    unaccomplished_list = [
        k.replace("_", " ") for k, v in info["achievements"].items() if v < 1
    ]
    accomplished_list = [
        k.replace("_", " ") for k, v in info["achievements"].items() if v > 0
    ]
    # print(unaccomplished_list)
    new_CTXT += "\nCurrent *accomplished* achievements: " + ", ".join(accomplished_list)
    new_CTXT += "\nCurrent *unaccomplished* achievements: " + ", ".join(
        unaccomplished_list
    )
    return new_CTXT
