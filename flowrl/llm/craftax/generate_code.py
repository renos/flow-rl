...
from flowrl.llm.craftax.verify_code import verify_function
import os
...
def generate_py_file(data, file_path="generated_file.py"):
    ...
    with open(file_path, "w") as f:
        f.write("from craftax.craftax.constants import *\n")
        f.write("from craftax.craftax.craftax_state import Inventory\n")
        f.write("import jax\n")
...
class Inventory:
    ...
    diamond: int = 0
    sapling: int = 0
    pickaxe: int = 0  # Tier indicator: 0=none, 1=wood, 2=stone, 3=iron, 4=diamond
    sword: int = 0  # Tier indicator: 0=none, 1=wood, 2=stone, 3=iron, 4=diamond
    bow: int = 0
    arrows: int = 0
    armour: jnp.ndarray = jnp.array([0, 0, 0, 0])  # Array of 4 armor pieces
    torches: int = 0
    ruby: int = 0
    sapphire: int = 0
    potions: jnp.ndarray = jnp.array([0, 0, 0, 0, 0, 0])  # Array of 6 potion types
    books: int = 0
...
def create_inventory_from_array(inventory_array):
    return Inventory(
        ...
        pickaxe=round(inventory_array[6]),
        sword=round(inventory_array[7]),
        bow=round(inventory_array[8]),
        arrows=round(inventory_array[9]),
        armour=jnp.array([round(inventory_array[10]), round(inventory_array[11]), round(inventory_array[12]), round(inventory_array[13])]),
        torches=round(inventory_array[14]),
        ruby=round(inventory_array[15]),
        sapphire=round(inventory_array[16]),
        potions=jnp.array([round(inventory_array[17]), round(inventory_array[18]), round(inventory_array[19]), round(inventory_array[20]), round(inventory_array[21]), round(inventory_array[22])]),
        books=round(inventory_array[23]),
    )