#!/bin/bash
# Run PPO Flow with TransformerXL on a single-node experiment: descend from Floor 0 to Floor 1

set -euo pipefail

echo "========================================"
echo "Run: One-Node Descend Floor (TXRL)"
echo "========================================"

# Configurable params (override via env vars)
ENV_NAME=${ENV_NAME:-Craftax-Symbolic-v1}
MODULE_PATH=${MODULE_PATH:-$(pwd)/exp/one_node_descend/1.py}
WANDB_PROJECT=${WANDB_PROJECT:-flow_rl_one_node_descend_txrl}
...
# Goal state after completing task_0 (descend)
