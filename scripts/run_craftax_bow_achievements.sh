#!/bin/bash
# Run PPO Flow on craftax_bow_achievements: collect wood → craft pickaxe → craft arrows → descend → open chest → kill monsters (bonus for orc mage/soldier)

set -euo pipefail

echo "========================================"
echo "Run: Craftax Bow w/ Achievements (wintrinsics, GAE=0.95, p=0.99)"
echo "========================================"

# Configurable params (override via env vars)
ENV_NAME=${ENV_NAME:-Craftax-Symbolic-v1}
MODULE_PATH=${MODULE_PATH:-$(pwd)/exp/craftax_bow_achievements/1.py}
WANDB_PROJECT=${WANDB_PROJECT:-flow_rl_craftax_bow_achievements}
...
# Goal state index for final task (task_5: kill monsters); 6 tasks total (0-5)
SUCCESS_STATE=${SUCCESS_STATE:-6}
GPU_ID=${GPU_ID:-1}
