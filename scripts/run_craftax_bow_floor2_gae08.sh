#!/bin/bash
# Run PPO Flow on craftax_bow_floor2 with progressive curriculum: collect wood → craft pickaxe → collect stone → craft arrows → descend floor 1 → open chest → kill monsters floor 1 → descend floor 2 → kill monsters floor 2

set -euo pipefail

echo "========================================"
echo "Run: Craftax Bow Floor 2 (Progressive Curriculum, GAE=0.8, p=0.99)"
echo "========================================"

# Configurable params (override via env vars)
ENV_NAME=${ENV_NAME:-Craftax-Symbolic-v1}
MODULE_PATH=${MODULE_PATH:-$(pwd)/exp/craftax_bow_floor2/1.py}
WANDB_PROJECT=${WANDB_PROJECT:-flow_rl_craftax_bow_floor2_gae08}
...
# Goal state index for final task (task_8: kill monsters on floor 2); 9 states total (0-8, success at 9)
SUCCESS_STATE=${SUCCESS_STATE:-9}
# Progressive curriculum settings
PROGRESSIVE_RESET_THRESHOLD=${PROGRESSIVE_RESET_THRESHOLD:-0.30}
