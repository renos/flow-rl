#!/bin/bash
# Run PPO Flow with TransformerXL on craftax_bow_and_sword with progressive curriculum: collect wood → craft pickaxe → collect stone → craft sword → craft arrows → descend floor 1 → open chest → kill monsters floor 1 → descend floor 2 → kill monsters floor 2

set -euo pipefail

echo "========================================"
echo "Run: Craftax Bow and Sword (TransformerXL, Progressive Curriculum, GAE=0.8, p=0.99)"
echo "========================================"

# Configurable params (override via env vars)
ENV_NAME=${ENV_NAME:-Craftax-Symbolic-v1}
MODULE_PATH=${MODULE_PATH:-$(pwd)/exp/craftax_bow_and_sword/1.py}
WANDB_PROJECT=${WANDB_PROJECT:-flow_rl_craftax_bow_and_sword_txrl}
...
# Goal state index for final task (task_9: kill monsters on floor 2); 10 states total (0-9, success at 10)
...
# TransformerXL-specific hyperparameters
...
# Run PPO Flow with TransformerXL
