...
# FILENAME_PREFIX = "collection_and_length_plot"
...
# LEGEND_COMBINED_BBOX_Y = -0.15  # lower center y for combined figure
...
import matplotlib.pyplot as plt
import seaborn as sns
from matplotlib.lines import Line2D  # <-- for legend spacer
...
def plot_metrics(
    ...
    # Legend (further down; more bottom padding) with spacer so first row has 2 items
    handles, labels = axes[-1].get_legend_handles_labels()
    if handles and labels:
        if LEGEND_OUTSIDE:
            # Insert a blank spacer to force first row to only have two methods (then 3 columns per row).
            if len(labels) >= 3 and LEGEND_NCOL >= 3:
                spacer = Line2D([], [], linestyle="None", linewidth=0, label="")
                handles = handles[:2] + [spacer] + handles[2:]
            ...
            fig.legend(
                ...
                labels,
                loc="lower center",
                ncol=min(LEGEND_NCOL, max(3, len(labels))),
                frameon=False,
                bbox_to_anchor=(0.5, LEGEND_COMBINED_BBOX_Y),
            )
            plt.subplots_adjust(bottom=BOTTOM_ADJUST_COMBINED)
    ...
    for metric in available_metrics:
        ...
        if LEGEND_OUTSIDE:
            handles, labels = ax.get_legend_handles_labels()
            if handles and labels:
                # Spacer to ensure first row has only two entries, then rows of 3
                if len(labels) >= 3 and LEGEND_NCOL >= 3:
                    spacer = Line2D([], [], linestyle="None", linewidth=0, label="")
                    handles = handles[:2] + [spacer] + handles[2:]
                    labels = labels[:2] + [""] + labels[2:]
