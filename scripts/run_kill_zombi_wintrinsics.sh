#!/bin/bash
# Run PPO Flow on a single-node experiment: descend from Floor 0 to Floor 1

set -euo pipefail

echo "========================================"
echo "Run: One-Node Descend Floor (PPO Flow)"
echo "========================================"

# Configurable params (override via env vars)
ENV_NAME=${ENV_NAME:-Craftax-Symbolic-v1}
MODULE_PATH=${MODULE_PATH:-$(pwd)/exp/kill_zombi_w_intrinsics/1.py}
WANDB_PROJECT=${WANDB_PROJECT:-flow_rl_kill_zombi_w_intrinsics}
