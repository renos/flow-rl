#!/bin/bash
# Run PPO Flow on craftax_bow_and_sword with progressive curriculum: collect wood → craft pickaxe → collect stone → craft sword → craft arrows → descend floor 1 → open chest → kill monsters floor 1 → descend floor 2 → kill monsters floor 2

set -euo pipefail

echo "========================================"
echo "Run: Craftax Bow and Sword (Progressive Curriculum, GAE=0.95, p=0.99)"
echo "========================================"
...
WANDB_PROJECT=${WANDB_PROJECT:-flow_rl_craftax_bow_and_sword}
